...
for (const { i, j, dimension } of results || []) {
  if (i >= 0 && j >= 0 && i <= N && j <= N && i + j <= N) {
    const dimBig = (typeof dimension === "bigint") ? dimension : BigInt(dimension);
    hij[i][j] = hij[i][j] + dimBig; // BigInt-safe addition
  }
}


  // Build 2N+1 rows. Row r shows h^{0,r}, h^{1,r-1}, ..., h^{r,0} (zeros when i+j > N).
  ...
  for (let i = iMin; i <= iMax; i++) {
    const j = r - i;
    const val = (i + j <= N) ? hij[i][j] : 0n; // BigInt

    const valueCell = document.createElement("span");
    valueCell.className = "diamond-value";
    valueCell.innerText = val.toString(); // exact integer text
